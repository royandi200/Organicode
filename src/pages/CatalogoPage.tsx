...
import {
  ...
  Coffee, Star, TrendingUp, ChevronDown
} from 'lucide-react';
import { lotes, precioActual } from '@/data/mock';
import { useAppStore } from '@/store';

/* ── Lote Card for Catalog ── */
function CatalogLoteCard({ lote, index }: { lote: typeof lotes[0]; index: number }) {
  const { openOfferModal, openSampleModal } = useAppStore();
  const scaColor = lote.sca_score >= 87 ? 'bg-huila-green/10 text-huila-green' : lote.sca_score >= 85 ? 'bg-volcanic-gold/10 text-volcanic-gold' : 'bg-text-muted/10 text-text-muted';
  const estadoConfig = {
    publicado: { label: 'DISPONIBLE', color: 'bg-huila-green/10 text-huila-green border-huila-green/30' },
    ofertado: { label: 'OFERTADO', color: 'bg-volcanic-gold/10 text-volcanic-gold border-volcanic-gold/30 animate-pulse-gold' },
  ...
  return (
    ...
    >
      <div className="aspect-[4/3] overflow-hidden relative">
        <img src={lote.foto_url} alt={lote.nombre} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500" />
      ...
      <div className="p-5">
        ...
        <p className="text-xs text-text-sand mb-3">{lote.variedad} · {lote.proceso} · {lote.municipio}</p>
        ...
        <div className="flex items-center justify-between">
          <div>
            <span className="font-mono-data text-xl font-bold text-volcanic-gold">${lote.precio_calculado.toFixed(2)}</span>
            <span className="text-xs text-text-sand">/kg</span>
          </div>
          <div className="flex gap-2">
            <button onClick={() => openSampleModal(lote)} className="px-3 py-1.5 text-xs text-text-sand border border-gold-subtle rounded-lg hover:border-volcanic-gold transition-colors">
              Muestra
            </button>
            <button onClick={() => openOfferModal(lote)} className="px-3 py-1.5 text-xs bg-volcanic-gold text-void rounded-lg hover:bg-volcanic-gold/90 transition-colors">
              Oferta
            </button>
          </div>
        </div>
  ...
  return (
    <div className="glass-panel rounded-xl p-6">
      <div className="flex items-center justify-between mb-6">
        ...
        <button onClick={() => setCollapsed(true)} className="text-text-muted hover:text-text-warm transition-colors">
          <X className="w-4 h-4" />
        </button>
      </div>
        ...
        <AnimatePresence mode="wait">
            ...
            <motion.div key="volumen" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }}>
              ...
              <div className="space-y-2">
                {volumenOptions.map(opt => (
                  <button
                    key={opt.value}
                    onClick={() => { setPrefs({ ...prefs, volumen: opt.value }); }}
          ...
          {step === 1 && (
            <motion.div key="perfil" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }}>
              <p className="text-sm text-text-sand mb-4">Selecciona uno o más (múltiple)</p>
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
                {perfilOptions.map(opt => (
                  <button
                    key={opt.value}
                    onClick={() => {
                      const newPerfil = prefs.perfil.includes(opt.value)
                        ? prefs.perfil.filter(p => p !== opt.value)
                        : [...prefs.perfil, opt.value];
            ...
            <motion.div key="sca" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }}>
              ...
              <div className="relative pt-6 pb-2">
                <div className="flex justify-between text-[10px] text-text-muted uppercase tracking-wider mb-2">
                  <span>82 Comercial</span>
                  <span>85 Premium</span>
                  <span>87 Specialty</span>
                  <span>95 Ultra</span>
                </div>
                <input
                  type="range"
      ...
      <div className="flex justify-between mt-6">
        {step > 0 && (
          <button onClick={() => setStep(step - 1)} className="text-sm text-text-sand hover:text-text-warm transition-colors">
            ← Anterior
          </button>
        )}
        <button
...
export function CatalogoPage() {
  ...
  const variedades = [...new Set(lotes.map(l => l.variedad))];
  const procesos = [...new Set(lotes.map(l => l.proceso))];
  const municipios = [...new Set(lotes.map(l => l.municipio))];
  ...
  const filteredLotes = useMemo(() => {
    return lotes.filter(lote => {
      ...
      if (search && !lote.nombre.toLowerCase().includes(search.toLowerCase()) && !lote.variedad.toLowerCase().includes(search.toLowerCase())) return false;
      ...
      if (lote.sca_score < filters.scaMin) return false;
      if (lote.precio_calculado > filters.precioMax) return false;
      if (filters.municipios.length && !filters.municipios.includes(lote.municipio)) return false;
      return true;
    }).sort((a, b) => {
      if (a.estado === 'vendido' && b.estado !== 'vendido') return 1;
      if (a.estado !== 'vendido' && b.estado === 'vendido') return -1;
      return b.sca_score - a.sca_score;
    });
  }, [filters, search]);

  const handleMatcherComplete = (prefs: any) => {
    setFilters(prev => ({
      ...prev,
      scaMin: prefs.scaMin || 82,
      precioMax: prefs.presupuesto || 100,
    }));
  };
        ...
        <div className="max-w-7xl mx-auto">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
            <div>
              ...
              <p className="text-sm text-text-sand">{filteredLotes.length} lotes disponibles · Precios actualizados</p>
            </div>
            <div className="flex items-center gap-3">
              <div className="glass-pill px-3 py-1.5 flex items-center gap-2">
                <TrendingUp className="w-3 h-3 text-huila-green" />
                <span className="font-mono-data text-xs text-text-warm">${precioActual.precio_ice.toFixed(2)} ICE</span>
              </div>
            </div>
          </div>
          ...
          {/* Coffee Matcher */}
          <div className="mb-8">
            <CoffeeMatcher onComplete={handleMatcherComplete} />
          </div>
                ...
                <div className="glass-panel rounded-xl p-5 grid sm:grid-cols-2 lg:grid-cols-4 gap-6">
                  ...
                  <div>
                    ...
                    <div className="flex flex-wrap gap-2">
                      {variedades.map(v => (
                        <button
                          key={v}
                          onClick={() => toggleFilter('variedades', v)}
                  ...
                  <div>
                    ...
                    <div className="flex flex-wrap gap-2">
                      {procesos.map(p => (
                        <button
                          key={p}
                          onClick={() => toggleFilter('procesos', p)}
                          className={`px-2.5 py-1 text-xs rounded-md border transition-colors ${
                  ...
                  <div>
                    ...
                    <input
                      ...
                      min={82}
                      max={95}
                      value={filters.scaMin}
                      onChange={(e) => setFilters({ ...filters, scaMin: Number(e.target.value) })}
                      className="w-full accent-volcanic-gold"
                    />
                    <div className="flex justify-between text-[10px] text-text-muted mt-1">
                      <span>82</span>
                      <span className="text-volcanic-gold font-mono-data">{filters.scaMin}</span>
                      <span>95</span>
                    </div>
                  </div>
                  ...
                  <div>
                    ...
                    <div className="flex flex-wrap gap-2">
                      {municipios.map(m => (
                        <button
                          key={m}
                          onClick={() => toggleFilter('municipios', m)}
                          className={`px-2.5 py-1 text-xs rounded-md border transition-colors ${
          ...
          {filteredLotes.length > 0 ? (
            ...
            <div className="text-center py-20">
              ...
              <p className="text-sm text-text-sand mb-4">Intenta ajustar tus filtros o búsqueda</p>
              <button
                onClick={() => { setFilters({ variedades: [], procesos: [], scaMin: 82, precioMax: 100, municipios: [] }); setSearch(''); }}
