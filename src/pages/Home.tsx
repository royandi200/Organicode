...
export default function Home() {
  ...
  const handleReg = (e: React.FormEvent) => {
    ...
    addToast({ type: 'sistema', title: '¡Bienvenido!', message: 'Tu solicitud de acceso ha sido enviada. Te contactaremos pronto.' });
  };
    ...
    <div className="min-h-screen">
        ...
        <div className="relative z-10 w-full px-4 sm:px-6 lg:px-8 text-center">
          ...
          >
            <div className="flex items-center justify-center gap-2 mb-6">
              <div className="glass-pill px-4 py-2 flex items-center gap-2">
                ...
                <span className="font-mono-data text-xs text-text-warm">
                  ICE ${precioActual.precio_ice.toFixed(2)} USD/lb
                </span>
            ...
            <h1 className="font-display text-5xl sm:text-6xl lg:text-7xl xl:text-8xl text-text-warm leading-[1.05] max-w-5xl mx-auto mb-6">
              El café más trazado del mundo nace aquí
            </h1>

            <p className="text-lg sm:text-xl text-text-sand max-w-2xl mx-auto mb-10">
              Comercialización directa de café gourmet del Huila. Precios en tiempo real, 
              trazabilidad blockchain y acceso a los mejores lotes de especialidad.
            </p>

            <div className="flex flex-col sm:flex-row gap-4 justify-center">
              <Link
                to="/catalogo"
                className="px-8 py-4 bg-volcanic-gold text-void font-medium rounded-xl hover:bg-volcanic-gold/90 hover:shadow-gold transition-all flex items-center justify-center gap-2"
              >
      ...
      <section className="py-24 px-4 sm:px-6 lg:px-8 bg-void">
        <div className="max-w-6xl mx-auto">
          <div className="grid md:grid-cols-3 gap-8">
            {[
              { icon: Shield, title: 'Origen verificado', desc: 'Cada lote trazado en blockchain desde la finca hasta tu tostadora. Registro inmutable en Polygon.', color: 'text-volcanic-gold' },
              { icon: Coins, title: 'Precio justo en tiempo real', desc: 'Precios calculados con datos del mercado NY ICE + diferenciales regionales + prima de calidad SCA.', color: 'text-huila-green' },
              { icon: Handshake, title: 'Del caficultor al tostador', desc: 'Sin intermediarios. Compra directa de productores del Huila con pagos justos y transparentes.', color: 'text-volcanic-gold' },
            ].map((item, i) => (
              <motion.div
                key={item.title}
              ...
              >
                ...
                <h3 className="font-display text-xl text-text-warm mb-3">{item.title}</h3>
                <p className="text-sm text-text-sand leading-relaxed">{item.desc}</p>
              </motion.div>
            ))}
          </div>
        </div>
      ...
      <section className="py-24 px-4 sm:px-6 lg:px-8 bg-parchment">
        <div className="max-w-6xl mx-auto">
          ...
          >
            <p className="text-xs uppercase tracking-widest text-text-muted mb-3">Disponibles ahora</p>
            <h2 className="font-display text-4xl text-text-ink">Lotes Destacados</h2>
          </motion.div>

          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
            {publicados.map((lote, i) => (
          ...
          <div className="text-center mt-10">
            <Link
              to="/catalogo"
              className="inline-flex items-center gap-2 px-6 py-3 border border-gold-medium text-volcanic-gold rounded-xl hover:bg-volcanic-gold/10 transition-colors"
            >
      ...
      <section className="py-24 px-4 sm:px-6 lg:px-8 bg-void">
        <div className="max-w-5xl mx-auto">
          ...
          >
            <h2 className="font-display text-4xl text-text-warm mb-3">¿Cómo funciona?</h2>
            <p className="text-text-sand">De la finca a tu bodega en 5 pasos simples</p>
          </motion.div>

          <div className="grid grid-cols-2 md:grid-cols-5 gap-8">
            <Step number={1} title="Registro" desc="Crea tu cuenta como comprador internacional" icon={FileCheck} delay={0} />
            <Step number={2} title="Explora" desc="Navega lotes con filtros avanzados y Coffee Matcher" icon={Search} delay={0.1} />
            <Step number={3} title="Analiza" desc="Revisa perfiles sensoriales, SCA y trazabilidad blockchain" icon={BarChart3} delay={0.2} />
            <Step number={4} title="Oferta" desc="Envía tu oferta formal con precio e incoterm deseados" icon={DollarSign} delay={0.3} />
            <Step number={5} title="Recibe" desc="Logística coordinada. De la finca a tu tostadora" icon={Truck} delay={0.4} />
          </div>
        </div>
      </section>
      ...
      <section className="py-24 px-4 sm:px-6 lg:px-8 bg-void relative overflow-hidden">
        ...
        <div className="max-w-6xl mx-auto relative z-10">
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-8 text-center">
            {[
              { value: 24, suffix: '', label: 'Caficultores activos' },
              { value: 4, suffix: '', label: 'Países compradores' },
              { value: 90, suffix: '+', label: 'Puntos SCA máximo' },
              { value: 100, suffix: '%', label: 'Trazable desde la finca' },
            ].map((stat, i) => (
              <motion.div
                key={stat.label}
                initial={{ opacity: 0, y: 20 }}
      ...
      <section className="py-24 px-4 sm:px-6 lg:px-8 bg-parchment">
        <div className="max-w-6xl mx-auto">
          <motion.div
            ...
            className="text-center mb-14"
          >
            <h2 className="font-display text-4xl text-text-ink mb-3">Voces del Huila</h2>
            <p className="text-text-muted">Lo que dicen nuestros caficultores</p>
          </motion.div>

          <div className="grid md:grid-cols-3 gap-6">
            {[
              {
                quote: "Organicode nos dio visibilidad internacional que nunca imaginamos. Ahora vendemos directamente a tostadores en Corea y Alemania.",
                name: "Alexander Vásquez",
                farm: "Finca El Encanto",
                location: "Baraya, Huila",
                image: "/images/producer-portrait.jpg",
              },
              {
                quote: "El precio justo cambió nuestra realidad. Dejamos de depender de los intermediarios y ahora recibimos el verdadero valor de nuestro trabajo.",
                name: "María Elena Torres",
                farm: "Finca La Montaña",
                location: "Baraya, Huila",
                image: "/images/hands-cherries.jpg",
              },
              {
                quote: "La trazabilidad blockchain le da confianza a nuestros compradores. Saben exactamente de dónde viene cada grano de café.",
                name: "Carlos Mario Ríos",
                farm: "Finca San Judas",
              ...
              <motion.div
                ...
                viewport={{ once: true }}
                transition={{ delay: i * 0.1, duration: 0.5 }}
                className="bg-white rounded-xl p-6 border border-gold-subtle/50"
              >
                <p className="text-text-ink/80 text-sm leading-relaxed mb-6 italic">"{t.quote}"</p>
                <div className="flex items-center gap-3">
                  <img src={t.image} alt={t.name} className="w-10 h-10 rounded-full object-cover" />
                  <div>
                    <p className="text-sm font-medium text-text-ink">{t.name}</p>
                    <p className="text-xs text-text-muted">{t.farm} · {t.location}</p>
                  </div>
                </div>
              </motion.div>
            ))}
      ...
      <section className="py-24 px-4 sm:px-6 lg:px-8 bg-void">
        <div className="max-w-xl mx-auto text-center">
          ...
          >
            ...
            <h2 className="font-display text-4xl text-text-warm mb-4">Accede al catálogo exclusivo</h2>
            <p className="text-text-sand mb-10">
              Regístrate como comprador internacional y obtén acceso a lotes de especialidad 
              con precios en tiempo real y trazabilidad completa.
            </p>
            ...
            {regSubmitted ? (
              <div className="bg-huila-green/10 border border-huila-green/30 rounded-xl p-6">
                ...
                <h3 className="font-display text-xl text-text-warm mb-2">¡Solicitud enviada!</h3>
                <p className="text-sm text-text-sand">Te contactaremos en menos de 24 horas con acceso al catálogo.</p>
              </div>
            ) : (
              <form onSubmit={handleReg} className="space-y-3">
                <div className="grid sm:grid-cols-2 gap-3">
                  <input
                    ...
                    className="w-full bg-surface border border-gold-subtle rounded-lg px-4 py-3 text-sm text-text-warm placeholder:text-text-muted focus:border-volcanic-gold focus:outline-none transition-colors"
                  />
                  <input
                    type="text"
                    required
                ...
                <div className="grid sm:grid-cols-2 gap-3">
                  ...
                  <input
                    ...
                    placeholder="País"
                    value={regForm.pais}
                    onChange={(e) => setRegForm({ ...regForm, pais: e.target.value })}
                    className="w-full bg-surface border border-gold-subtle rounded-lg px-4 py-3 text-sm text-text-warm placeholder:text-text-muted focus:border-volcanic-gold focus:outline-none transition-colors"
                  />
                </div>
                <button
                  type="submit"
                  className="w-full py-4 bg-volcanic-gold text-void font-medium rounded-xl hover:bg-volcanic-gold/90 hover:shadow-gold transition-all flex items-center justify-center gap-2"
                >
                  Quiero acceder al catálogo
                  <ArrowRight className="w-4 h-4" />
                </button>
              </form>
            )}
        ...
        <div className="max-w-6xl mx-auto">
          <div className="flex flex-col md:flex-row items-center justify-between gap-6">
            ...
            <p className="text-xs text-text-muted">
              © 2026 Organicode. Todos los derechos reservados.
            </p>
          </div>
        </div>
      </footer>
