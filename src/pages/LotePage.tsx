...
import { getLoteBySlug, calcularDesglosePrecio, precioActual, incotermRates } from '@/data/mock';
import { useAppStore } from '@/store';
import { FlavorWheel } from '@/components/FlavorWheel';

export function LotePage() {
  const { slug } = useParams<{ slug: string }>();
  const lote = getLoteBySlug(slug || '');
  ...
  useEffect(() => {
    const onScroll = () => {
      if (heroRef.current) {
        setShowSticky(window.scrollY > heroRef.current.offsetHeight - 100);
      }
    };
  ...
  const desglose = calcularDesglosePrecio(lote.sca_score, precioActual);
  ...
  const activeNotes = lote.notas_sensoriales.split(',').map(n => n.trim());
    ...
    <div className="min-h-screen bg-parchment">
      ...
      <section ref={heroRef} className="relative min-h-screen flex items-center justify-center overflow-hidden">
        ...
        <div className="relative z-10 w-full px-4 sm:px-6 lg:px-8 pt-24 pb-16">
          ...
          >
            ...
            <button
              onClick={copyHash}
              className="inline-flex items-center gap-2 glass-pill px-4 py-2 mb-6 hover:border-volcanic-gold/50 transition-colors group"
            >
              ...
              <span className="text-[10px] font-mono-data text-text-muted hidden sm:inline">
                {lote.hash_registro ? `· ${lote.hash_registro.slice(0, 10)}...` : '· Trazabilidad en proceso'}
              </span>
              {copiedHash ? (
                <Check className="w-3.5 h-3.5 text-huila-green" />
              ) : (
                <Copy className="w-3.5 h-3.5 text-text-muted group-hover:text-volcanic-gold transition-colors" />
              )}
            </button>

            {/* Title */}
            <h1 className="font-display text-5xl sm:text-6xl lg:text-7xl text-text-ink leading-[1.05] mb-3">
              {lote.nombre}
            </h1>
            ...
            <div className="flex items-center justify-center gap-6 sm:gap-8 text-sm text-text-ink/70 mb-10">
              ...
              <span className="flex items-center gap-1.5">
                ...
                {lote.municipio}, Huila
              </span>
              <span className="text-text-muted hidden sm:inline">·</span>
              <span className="hidden sm:inline">
                <TrendingUp className="w-4 h-4 inline mr-1" />
                {lote.altitud_msnm.toLocaleString()} msnm
              </span>
            </div>
            ...
            >
              <div className="flex items-center justify-between mb-2">
                <span className="font-mono-data text-3xl font-bold text-volcanic-gold">
                  ${precioFinal.toFixed(2)}
                </span>
                <span className="text-xs text-text-sand uppercase tracking-wider">USD/kg {incoterm}</span>
              </div>
              <div className="flex items-center gap-2 text-xs text-text-muted">
                <TrendIcon className={`w-3 h-3 ${precioActual.trend === 'up' ? 'text-huila-green' : 'text-warm-rust'}`} />
                ...
                {(['EXW', 'FOB', 'CIF'] as const).map((term) => (
                  <button
                    ...
                    className={`flex-1 py-2 text-xs font-medium rounded-lg transition-all duration-200 ${
                      incoterm === term
                        ? 'bg-volcanic-gold text-void'
                        : 'bg-surface text-text-sand hover:text-text-warm border border-gold-subtle'
                    }`}
                  >
                    {term}
                  </button>
              ...
              {incoterm === 'CIF' && (
                <select
                  value={cifCountry}
                  onChange={(e) => setCifCountry(e.target.value)}
        ...
        <div className="max-w-6xl mx-auto">
          ...
          >
            ...
            <div className="lg:col-span-2 rounded-2xl overflow-hidden border border-gold-subtle h-[400px] relative bg-surface">
              <iframe
                src={`https://www.google.com/maps/embed?pb=!1m14!1m12!1m3!1d15949.!2d${lote.coordenadas.lng}!3d${lote.coordenadas.lat}!2m3!1f0!2f0!3f0!3m2!1i1024!2i768!4f13.1!5e1!3m2!1ses!2sco!4v1`}
              ...
              <div className="absolute top-4 left-4 glass-pill px-3 py-1.5">
                <span className="text-xs font-medium text-text-warm flex items-center gap-1.5">
                  <MapPin className="w-3 h-3 text-volcanic-gold" />
                  {lote.municipio}, Huila
                </span>
              </div>
            </div>
            ...
            <div className="space-y-4">
              ...
              {[
                { icon: MapPin, label: 'Municipio', value: lote.municipio },
      ...
      <section className="py-20 px-4 sm:px-6 lg:px-8 bg-void">
        <div className="max-w-6xl mx-auto">
          ...
          >
            ...
            <p className="text-text-sand max-w-xl mx-auto">
              Análisis Q-Grader certificado. Cada nota representa una característica identificada en la cata profesional.
            </p>
          </motion.div>
          ...
          <div className="grid lg:grid-cols-2 gap-12 items-center">
            ...
            <div className="space-y-6">
              ...
              <div className="glass-panel rounded-xl p-5 space-y-3">
                ...
                {[
                  ['Proceso', `${lote.proceso} · ${lote.cosecha}`],
          ...
          <div className="grid lg:grid-cols-2 gap-12 items-center">
            ...
            >
              <div className="relative">
                <img
                  src={lote.caficultor.foto}
                  alt={lote.caficultor.nombre}
            ...
            >
              <div>
                ...
                <h2 className="font-display text-3xl sm:text-4xl text-text-ink mb-4">
                  {lote.caficultor.nombre}
                </h2>
              </div>
              ...
              <div className="flex items-center gap-3 text-sm text-text-ink/70">
                ...
                <span>·</span>
                <span>Vda. {lote.caficultor.vereda}, {lote.caficultor.municipio}</span>
                <span>·</span>
                <span>desde {lote.caficultor.desde}</span>
              </div>

              <p className="text-text-ink/80 leading-relaxed text-lg">
                {lote.storytelling_ia}
              </p>

              {/* Social Value Badges */}
              <div className="flex flex-wrap gap-3 pt-4">
                {lote.caficultor.valores.map((v) => {
                  ...
                  return (
                    <span
                      key={v}
                      className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-white/60 rounded-lg text-xs font-medium text-text-ink border border-gold-subtle/30"
                    >
                      <Icon className="w-3.5 h-3.5 text-volcanic-gold" />
                      {vm.label}
                    </span>
                  );
                })}
      ...
      <section className="py-20 px-4 sm:px-6 lg:px-8 bg-void">
        <div className="max-w-4xl mx-auto">
          ...
          >
            <h2 className="font-display text-3xl sm:text-4xl text-text-warm mb-3">
              La cadena de confianza de este lote
            </h2>
            <p className="text-text-sand">
              Cada etapa registrada en blockchain para garantizar trazabilidad completa.
            </p>
          </motion.div>
          ...
          {lote.hash_registro && (
            ...
            >
              <a
                href={`https://polygonscan.com/tx/${lote.hash_registro}`}
                target="_blank"
                rel="noopener noreferrer"
                className="inline-flex items-center gap-2 px-5 py-3 bg-volcanic-gold/10 border border-volcanic-gold/40 rounded-xl text-volcanic-gold hover:bg-volcanic-gold/20 transition-colors"
              >
                <Hexagon className="w-4 h-4" />
                <span className="text-sm font-medium">Ver en Blockchain</span>
        ...
        <div className="max-w-2xl mx-auto">
          ...
          >
            ...
            <p className="text-text-muted text-center text-sm mb-10">
              Cada componente del precio explicado al detalle
            </p>
            ...
            <div className="bg-white rounded-2xl border border-gold-subtle/50 p-6 sm:p-8 space-y-4">
              {[
                ...
                <div key={item.label} className="flex justify-between items-center">
                  <span className={`text-sm ${item.highlight ? 'text-text-ink font-medium' : 'text-text-muted'}`}>
                    {item.label}
                  </span>
                  <span className={`font-mono-data text-sm ${item.highlight ? 'text-volcanic-gold font-semibold' : 'text-text-ink'}`}>
                    {item.isBase ? '' : '+'}{item.isBase ? '' : '$'} {item.isBase ? `$ ${item.value.toFixed(2)}` : item.value.toFixed(2)} USD/kg
                  </span>
                </div>
              ))}
              ...
              <div className="border-t border-gold-subtle/50 pt-4 mt-4">
                <div className="flex justify-between items-center">
                  ...
                  <span className="font-mono-data text-xl font-bold text-volcanic-gold">
                    $ {desglose.totalEXW.toFixed(2)} USD/kg
                  </span>
                </div>
              </div>

              {/* Incoterm total */}
              <div className="bg-void/5 rounded-xl p-4 mt-4">
                <div className="flex items-center justify-between mb-3">
                  <span className="text-sm font-medium text-text-ink">Incoterm seleccionado</span>
                    ...
                    {(['EXW', 'FOB', 'CIF'] as const).map((term) => (
                      <button
                        ...
                        className={`px-3 py-1 text-xs rounded-md transition-all ${
                          incoterm === term ? 'bg-volcanic-gold text-white' : 'bg-white text-text-muted hover:text-text-ink'
                        }`}
                      >
                        {term}
                      </button>
                ...
                {incoterm === 'CIF' && (
                  <select
                    value={cifCountry}
                    onChange={(e) => setCifCountry(e.target.value)}
                    className="w-full mb-3 bg-white border border-gold-subtle rounded-lg px-3 py-2 text-sm text-text-ink"
                  >
                    <option>Alemania</option>
                    <option>Corea del Sur</option>
                    <option>Japón</option>
                ...
                <div className="flex justify-between items-center pt-2 border-t border-gold-subtle/30">
                  <span className="text-sm text-text-muted">Total {incoterm}</span>
                  <span className="font-mono-data text-lg font-bold text-text-ink">
                    $ {precioFinal.toFixed(2)} USD/kg
                  </span>
                </div>
              </div>
            </div>
            ...
            <div className="w-full px-4 sm:px-6 lg:px-8 py-3">
              <div className="flex items-center justify-between gap-4">
                ...
                <div className="flex gap-2 shrink-0">
                  <button
                    onClick={() => openSampleModal(lote)}
                    className="px-3 py-2 bg-void border border-gold-subtle rounded-lg text-xs font-medium text-text-warm hover:border-volcanic-gold transition-colors whitespace-nowrap"
                  >
                  ...
                  <button
                    onClick={() => openOfferModal(lote)}
                    className="px-3 py-2 bg-volcanic-gold text-void rounded-lg text-xs font-medium hover:bg-volcanic-gold/90 transition-colors whitespace-nowrap"
                  >
